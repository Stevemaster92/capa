...
E_ABORT_TIMEOUT = -20
E_INVALID_UNICODE = -21
E_UNKNOWN = -22

# Regex to remove ANSI sequences generated by termcolor in the render modules.
...
def collect_samples(sample: str) -> List[str]:
    """
    collect and create a list of sample file paths to analyze.
    `sample` can either be a path pointing to a single file or a directory containing multiple files.
    in the case of a directory, recursively walk topdown the directory tree to collect all the sample file paths.

    args:
      sample (str): the path to the sample file or directory.
    """
...
def write_csv(result: str, timestamp: int):
    """
    write the result document to a CSV file.
    """
    filepath = "./capa-%s.csv" % timestamp
...
def log_error(code: int, msg: str, path: str, timestamp: int, csv: bool):
    """
...
def analyze_sample(args, sample: str, rules: RuleSet) -> Tuple[int, str, Dict[str, Any], MatchResults]:
    ...
    if format_ == FORMAT_AUTO:
        try:
            format_ = get_auto_format(sample)
        except UnsupportedFormatError:
            msg = log_unsupported_format_error()
    ...
    # various limitations
    try:
        file_extractors = get_file_extractors(sample, format_)
    ...
    if format_ == FORMAT_FREEZE:
        ...
        try:
            extractor = get_extractor(
                sample, format_, args.backend, sig_paths, should_save_workspace, disable_progress=args.quiet
            )
        except UnsupportedFormatError:
            msg = log_unsupported_format_error()
            return (E_INVALID_FILE_TYPE, msg, None, None)
        except UnsupportedArchError:
            msg = log_unsupported_arch_error()
...
def render_result(args, sample: str, meta, rules: RuleSet, capabilities: MatchResults, timestamp: int):
    # Decide how to render the result.
    if args.json:
        print(capa.render.json.render(meta, rules, capabilities))
    elif args.csv:
        csv = capa.render.csv.render(meta, rules, capabilities)

        # Write the result to a file instead of printing to the console.
        write_csv(csv, timestamp)
    elif args.vverbose:
    ...
    if args.log:
        ...
        filename = "%s-%s.clog" % (os.path.splitext(os.path.basename(sample))[0], timestamp)
...
def main(argv=None):
    ...
    epilog = textwrap.dedent(
        ...
        examples:
          ...
          identify capabilities in binaries located in a directory
            capa suspicious/
    ...
    parser.add_argument("-l", "--log", action="store_true", help="write result document(s) to log file(s)")
    ...
    spinner = halo.Halo(text_color="white", stream=sys.stderr)
